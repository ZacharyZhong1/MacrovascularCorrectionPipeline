...
OutputDirGeneral='../Simulated_Signal/R1/';
...
alpha=deg2rad(90);       % Flip angle for GRE (deg)
...
%% Input Signal
SigPost=sin(2*pi*0.1*(23:0.01:240-1));
SigPost=SigPost(rem(23:0.01:240-4.5,4.5)==0);
NumTime=length(SigPost);
...
for Sub=1:NumSub
    ...
    BloodSignal=(sin(alpha)*(1-exp(-TR./(T1Blood)))./(1-cos(alpha).*exp(-TR./(T1Blood))));
    BrainSignal=(sin(alpha)*(1-exp(-TR./(T1Tissue)))./(1-cos(alpha).*exp(-TR./(T1Tissue))));   
    ...
    for Time=1    
        ...
        OutputFile.vol(PaddingBOLD(3)+1:PaddingBOLD(4)+1,PaddingBOLD(1)+1:PaddingBOLD(2)+1,PaddingBOLD(5)+1:PaddingBOLD(6)+1)...
            =DecaySignal;
        MRIwrite(OutputFile,[OutputDir '/R1Iteration_',num2str(1),'Frame_',num2str(Time),'.nii.gz']);
        clear DecaySignal
    end
end
