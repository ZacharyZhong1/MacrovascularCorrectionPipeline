...
if [ ! -f ${image}_mask.${ext} ]; then
	if [ "${imgType}" = "TOF" ]; then
		...
		fsl5.0-bet ${image}_autobox.${ext} ${image}_ss.${ext} -m -f 0.2
		...
		if [ ! -f ${image}_mask.${ext} ]; then
			if [ -f ${phase}.${ext} ] ; then
				...
				fsl5.0-bet ${image}_autobox.${ext} ${image}_ss.${ext} -A2 ${phase}_weird.${ext} -R -m -f 0.3
				mv ${image}_ss_mask.${ext} ${image}_mask.${ext}
			else
				fsl5.0-bet ${image}_autobox.${ext} ${image}_ss.${ext} -R -m -f 0.65
				mv ${image}_ss_mask.${ext} ${image}_mask.${ext}
			fi
		else
...
if [ ! -f ${image}_newVed_unscaled.${ext} ]; then
	...
	3dTcat -prefix ${image}_Ved_scales_nofilters/Ved_scales.${ext}  ${image}_Ved_scales_nofilters/Scale_NOW*
	#rm -rf Scale_NOW*
	cp ${image}_Ved_scales_nofilters/Ved_scales.${ext} ${image}_Ved_scales.${ext}
	...
	3dTcat -prefix ${image}_Ved_scales/newVed_scales.${ext} ${image}_Ved_scales/Scale_rescaled_*
	#rm -rf Scale_NOW*
	cp ${image}_Ved_scales/newVed_scales.${ext} ${image}_newVed_scales.${ext}
	...
	3dTcat -prefix ${image}_Ved_rescales/newVed_notscaled_scales.${ext} ${image}_Ved_rescales/Scale_processed_*
	#rm -rf Scale_NOW*
	cp ${image}_Ved_rescales/newVed_notscaled_scales.${ext} ${image}_newVed_notscaled_scales.${ext}
	...
	if [ "${imgType}" = "SWI" ]; then
		...
		3dmask_tool -overwrite -input ${image}_mask_up.${ext} -prefix ${image}_newmask.${ext} -dilate_input -2
	elif [ "${imgType}" = "ToF" ]; then
		3dresample -overwrite -master ${image}_newVed_scales.${ext} -rmode Cu -prefix ${image}_mask_up.${ext} -inset ${image}_mask.${ext}
		3dmask_tool -overwrite -input ${image}_mask_up.${ext} -prefix ${image}_newmask.${ext} -dilate_input -1
	else
